...
- **`success_empty()` was missing from `OcpiResponse` in PR #31's branch** — it was added in PR #24's branch but not in PR #31's. When basing new work on PR #31, add `success_empty()` before using it in PUT/PATCH handlers.
- **`axum::routing::{put, patch}` free functions are NOT needed to chain methods on `MethodRouter`** — `.put(handler)` on a `MethodRouter` (returned by `get(handler)`) is a method call on the `MethodRouter`, not the free `put` function from `axum::routing`. Only import the free routing functions you use to START a route.
- **`HeaderMap::insert` requires `HeaderName`, not `&str`** — for custom headers like `x-total-count`, use `HeaderName::from_static("x-total-count")` (must be lowercase, static). Do not use the `HEADER_X_TOTAL_COUNT` `&str` constant directly as a map key.
- **OCPI Locations receiver URL includes `country_code` + `party_id` as path segments** — `{endpoint}/{cc}/{pid}/{lid}[/{evse_uid}][/{cid}]` per `mod_locations.asciidoc §Receiver Interface`. They are NOT just routing headers; they are actual URL path components.
- **Adding `serde`, `serde_json`, `chrono` as direct deps triggers `needs-human`** even when already in Cargo.lock transitively. These are workspace deps (no Cargo.lock change), but touching any `Cargo.toml` counts as a dependency change under the workflow rules.
- **`#![warn(missing_docs)]` + `-D warnings` requires `///` on every public enum variant.** The crate-level lint makes missing variant docs a compile error under clippy's `-D warnings`. Budget ~1 line per variant when estimating LOC for large enums like `ConnectorType` (43 variants = ~50 extra lines just for minimal docs).
- **`ConnectorType` has spec-defined mixed-case names** (`IEC_60309_2_single_16`, not all-caps). Use explicit `#[serde(rename = "...")]` on every variant — do NOT rely on `#[serde(rename_all)]` for this enum. The SCREAMING_SNAKE_CASE convention only applies to enums where all variants are all-caps in the spec.
- **`TokenType` is forward-referenced by `PublishTokenType` in the Locations module** before the Tokens module is implemented. Define it in `v2_2_1.rs` and note it will be shared when the Tokens module is added. Do NOT define it in `common.rs` (it's not truly cross-version common).
- **`clap_lex ≥ 1.1.0` (and `clap ≥ 4.6`) require Rust 1.85** because they use Cargo edition 2024. If the workspace MSRV CI job fails with `feature 'edition2024' is required`, bump `rust-version` in workspace `Cargo.toml` to `"1.85"` and update the CI job toolchain version to match.
- **`icu_* v2.2.0` (via `idna → reqwest`) require Rust 1.86.** If the MSRV CI job fails with `rustc 1.85.0 is not supported by icu_collections@2.2.0`, the real MSRV floor is 1.86. Bump `rust-version` to `"1.86"` and the CI toolchain to `@1.86.0`. Always re-check `cargo check` under the target toolchain before shipping MSRV CI changes.
- **Single designated branch → PRs accumulate across the session.** GitHub forbids opening a second PR from the same branch to the same base while one is already open. When a session picks multiple issues, later commits append to the existing PR. Workaround: update the PR title/description to close all issues, note the stacking in the PR body and JOURNAL, and ensure `needs-human` is applied if any commit triggers it.
