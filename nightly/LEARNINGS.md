...
- **`SCREAMING_SNAKE_CASE` does NOT insert `_` before digits.** `Ac1Phase` → `"AC1_PHASE"`, not `"AC_1_PHASE"`. `Iec62196T1` → `"IEC62196_T1"`, not `"IEC_62196_T1"`. Any enum variant name containing a digit that butts up against a word boundary (like `NEMA_5_20`, `AC_1_PHASE`, `IEC_62196_T1`) MUST use an explicit `#[serde(rename = "...")]`. Never rely on `rename_all` for digit-containing wire names.
- **Forward-reference enums (ConnectorType, ConnectorFormat, PowerType)** — when a struct in module A needs types from module B (which is in a pending PR), define them as forward references in module A with a doc comment noting the dependency. Once the pending PR merges, deduplicate by re-exporting from module B. Pattern used for `CdrLocation` needing Locations-module enums while PR #31 was pending.
